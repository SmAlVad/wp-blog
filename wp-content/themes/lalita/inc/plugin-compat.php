...
add_action( 'after_setup_theme', 'lalita_setup_woocommerce' );
/**
 * Set up WooCommerce
 *
 */
function lalita_setup_woocommerce() {
	...
	add_action( 'woocommerce_sidebar', 'lalita_construct_sidebars' );
}

if ( ! function_exists( 'lalita_woocommerce_start' ) ) {
	add_action( 'woocommerce_before_main_content', 'lalita_woocommerce_start', 10 );
	/**
	 * Add WooCommerce starting wrappers
	 *
	 */
	function lalita_woocommerce_start() { ?>
...
if ( ! function_exists( 'lalita_woocommerce_end' ) ) {
	add_action( 'woocommerce_after_main_content', 'lalita_woocommerce_end', 10 );
	/**
	 * Add WooCommerce ending wrappers
	 *
	 */
	function lalita_woocommerce_end() { ?>
						</div><!-- .entry-content -->
						<?php
						/**
...
if ( ! function_exists( 'lalita_woocommerce_css' ) ) {
	add_action( 'wp_enqueue_scripts', 'lalita_woocommerce_css', 100 );
	/**
	 * Add WooCommerce CSS
	 *
	 */
	function lalita_woocommerce_css() {
		if ( ! class_exists( 'WooCommerce' ) ) {
			return;
		}

		$mobile = apply_filters( 'lalita_mobile_media_query', '(max-width:768px)' );
...
if ( ! function_exists( 'lalita_bbpress_css' ) ) {
	add_action( 'wp_enqueue_scripts', 'lalita_bbpress_css', 100 );
	/**
	 * Add bbPress CSS
	 *
	 */
	function lalita_bbpress_css() {
...
if ( ! function_exists( 'lalita_buddypress_css' ) ) {
	add_action( 'wp_enqueue_scripts', 'lalita_buddypress_css', 100 );
	/**
	 * Add BuddyPress CSS
	 *
	 */
	function lalita_buddypress_css() {
...
if ( ! function_exists( 'lalita_beaver_builder_css' ) ) {
	add_action( 'wp_enqueue_scripts', 'lalita_beaver_builder_css', 100 );
	/**
	 ...
	 *
	 */
	function lalita_beaver_builder_css() {
